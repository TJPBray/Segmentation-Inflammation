...
auTrainingDS = transform(trainingDS,@segmentationAugmentationPipeline);
...
numClasses = 2;
encoderDepth=5; %Can have a deeper network than with MNIST to reflect larger images
...
trainingimage=readimage(imds,i);
labelimage = uint16(readimage(labelds,i));

predImage = prediction(:,:,1,i);
